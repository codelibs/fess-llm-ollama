...
public class OllamaLlmClientRetryTest extends UnitFessTestCase {

    /**
     * Verifies that {@link OllamaLlmClient#executeWithRetry} invokes
    ...
    public void test_executeWithRetry_invokesOnRetryBetweenAttempts() throws Exception {
        ...
        try {
            client.executeWithRetry("test", () -> {
                ...
                throw new OllamaLlmClient.RetryableHttpException(500, "boom");
            }, cb);
        ...
        for (final Throwable cause : seenCauses) {
            assertTrue("cause should be RetryableHttpException, got: " + cause, cause instanceof OllamaLlmClient.RetryableHttpException);
        }
    }
    ...
    public void test_executeWithRetry_nullCallback_doesNotThrow() throws Exception {
        ...
        try {
            client.executeWithRetry("chat", () -> {
                attempts.incrementAndGet();
                throw new OllamaLlmClient.RetryableHttpException(503, "unavailable");
            });
            fail("expected IOException after retries are exhausted");
        } catch (final IOException expected) {
            // ok
